    ...
    for size in range(1, len(available_slots) + 1):
        for slot_combo in combinations(available_slots, size):
            ...
            for levels in product(*ranges):
                ...
                predicate = {SLOT_PROPERTY_NAME[slot]: level for slot, level in slot_levels.items()}
                overrides.append({"predicate": predicate, "model": f"{MODID}:item/{model_name}"})
                write_json(MOD_MODEL_DIR / f"{model_name}.json", make_model(parent, item_name, slot_levels))
    root_model = {
        "parent": parent,
        "textures": {"layer0": f"minecraft:item/{item_name}"},
        "overrides": overrides,
